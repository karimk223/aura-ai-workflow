...
export default function ReviewDashboard({
  ...
  async function submitDecision(piece: Piece, decision: Decision) {
    if (
      decision === 'no_good' &&
      !window.confirm(`Send “${piece.pieceName}” to CreatorRejected?`)
    ) {
      return;
    }
    ...
    try {
      ...
      setNotice(
        decision === 'good'
          ? `${piece.pieceName} moved to CreatorApproved.`
          : `${piece.pieceName} moved to CreatorRejected.`,
      );
    } catch (decisionError) {
      setError(
  ...
  return (
    <main className="site-shell">
      <header className="topbar">
        <a className="brand" href="#top" aria-label="AURA content review home">
          <img className="brand-logo" src="/aura-logo.png" alt="AURA by Nada" />
          <span className="brand-section">Content Review</span>
        </a>
      ...
      <section className="hero" id="top">
        <div>
          <p className="eyebrow">Creator approval queue</p>
          <h1>Review each product set with confidence.</h1>
          <p className="hero-copy">
            Compare all four AI-approved views, then send the complete piece to the
            correct next stage with one decision.
          </p>
        </div>
      ...
      <section className="queue" aria-live="polite" aria-busy={loading}>
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p>New AI-approved pieces will appear here automatically.</p>
            <button
              type="button"
          ...
          pieces.map((piece, pieceIndex) => (
            <article className="piece-card" key={piece.pieceFolderId}>
              ...
              <div className="decision-bar">
                <div>
                  <strong>Creator decision</strong>
